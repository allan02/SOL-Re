...
def show_business_case_analysis():
    ...
    def analyze_groups_node(state: AnalysisState) -> AnalysisState:
        ...
        try:
            prompt = ChatPromptTemplate.from_messages([
                ("system",
                    "너는 스테이블 코인 관련 뉴스 기사들을 분석하여 신한은행 입장에서의 종합적인 인사이트를 제공하는 전문가다. \n"
                    ...
                    "**신한은행 입장에서의 영향도 판단**\n"
                    "이 뉴스 이슈가 신한은행에게 미치는 영향을 다음 중 하나로 판단:\n"
                    "- 매우 부정적: 신한은행의 사업에 심각한 위협이나 손실을 가져올 수 있는 이슈\n"
                    "- 부정적: 신한은행의 사업에 부정적 영향을 미칠 수 있는 이슈\n"
                    "- 중립: 신한은행의 사업에 직접적인 영향이 미미하거나 불분명한 이슈\n"
                    "- 긍정적: 신한은행의 사업에 긍정적 기회를 제공할 수 있는 이슈\n"
                    "- 매우 긍정적: 신한은행의 사업에 큰 성장 기회나 이익을 가져올 수 있는 이슈\n\n"
                    "**신한은행이 얻을 수 있는 구체적인 인사이트** (400자 이내)\n"
                    "1. 경쟁사 분석: 경쟁사들의 구체적인 움직임, 전략, 시장 포지셔닝\n"
                    "2. 위험 요소: 주의해야 할 구체적인 리스크, 규제 변화, 시장 불확실성\n"
                    "3. 시장 포지셔닝: 신한은행이 차별화할 수 있는 영역과 핵심 역량\n\n"
                    "**추천 조사 질의** (최대 2개)\n"
                    "해당 뉴스 기사들의 어떠한 문구을 고려했을 떄, 신한은행 입장에서 추가적으로 조사가 필요한 주제로 질문을 만들어라."
                    "질의는 완성된 한 개의 문장 형식으로 만들어라."
                    "질의는 구체적일 수록 좋다."
                    "각 섹션은 명확하게 구분하고, 신한은행의 비즈니스 관점에서 실용적이고 실행 가능한 인사이트를 제공해라."),
                ("human",
                 "회사: {company}\n이슈 라벨: {label}\n기사 제목들: {titles}\n기사 요약들: {descriptions}\n위 내용을 종합 분석:")
            ])
        ...
        try:
            prompt = ChatPromptTemplate.from_messages([
                ("system",
                 ...
                 "3) 신한은행에 미치는 영향이 클수록 더 중요 (직접 영향도)\n"
                 "4) 신한은행이 벤치마킹할 여지가 많을수록 더 중요 (학습/적용 가능성)\n"
